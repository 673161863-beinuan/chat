...
public class Connection extends Thread {
	...
	public void run() {
		try {// obj是Object类的对象
			...
			if (obj.getClass().getName().equals("com.beinuan.chat.Customer")) {
				serverLogin();
			}
			if (obj.getClass().getName().equals("com.beinuan.chat.Register_Customer")) {
				serverRegiste();
			}
			if (obj.getClass().getName().equals("com.beinuan.chat.Message")) {
				serverMessage();
			}
			if (obj.getClass().getName().equals("com.beinuan.chat.Chat")) {
				serverChat();
			}
			if (obj.getClass().getName().equals("com.beinuan.chat.Exit")) {
				serverExit();
			}
		} catch (IOException e) {
	...
	public void serverRegiste() {
		try {
			...
			File fList = new File("config\\user.txt");
			// 判断是否是第一个注册用户
			if (fList.length() != 0) {
